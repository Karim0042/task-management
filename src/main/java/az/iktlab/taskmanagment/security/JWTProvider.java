package az.iktlab.taskmanagment.security;
...
public class JWTProvider {
    ...
    public JwtToken getJWTToken(String userId, Boolean is_remember) {
        Date createDate = DateHelper.now();
        Date expirationDate = is_remember ? getInfiniteExpirationDate() : getExpirationDate();
        String jwtToken = JWT
                .create()
                .withSubject("Course Management")
    ...
    private Date getInfiniteExpirationDate() {
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.HOUR, Integer.MAX_VALUE);
        return calendar.getTime();
    }
}
